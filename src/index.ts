...
keyword 추출 규칙 (매우 중요):
- keyword에는 **문서 제목만** 넣으세요. 부가 설명이나 하위 주제는 제외.
- 예: '깨비물거미 문서에서 실전 배틀에 대해 알려줘' → keyword: '깨비물거미'
- 예: '나무위키 이순신 업적' → keyword: '이순신'

응답 활용:
- 응답의 text는 문서 전체 내용을 포함하므로 AI가 직접 필요한 부분을 찾아 요약하세요.
- 사용자가 특정 주제를 물으면 긴 문서에서 해당 내용을 검색하여 답변하세요.`,
